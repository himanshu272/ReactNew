...
function Details(props) {
  ...
  return (
    <div className="jumbotron">
      <h3> Details: </h3>
      <img className="img-thumbnail" src={props.movie.Poster} alt={img} width="200"></img>
      <hr/>
      <h6 className="display-6"> <kbd>Movie name:</kbd> {props.movie.Title} </h6>
      <hr></hr>
      <h6 className="display-6"> <kbd>Released in:</kbd> {props.movie.Year}</h6>
      <hr/>
      <h6 className="display-6"> <kbd>Rated:</kbd> {props.movie.Rated}</h6><hr/>
      <h6 className="display-6"> <kbd>Plot:</kbd> {props.movie.Plot}</h6><hr/>
    </div>
  );
}
